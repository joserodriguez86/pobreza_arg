...
train <- eph_mod %>% 
  filter(ano_trim %in% c("2016-II", "2016-III", "2016-IV", "2017-I"))
...
#Preprocesamiento
recetas1 <- recipe(pobreza_dic ~ ., data = train) %>%
...
recetas2 <- recipe(pobreza_dic ~ ., data = train) %>%
  update_role(ano_trim, CODUSU, NRO_HOGAR, new_role = "ID") %>%
...
#Preprocesamiento
recetas <- recipe(pobreza_dic ~ ., data = train) %>%
  update_role(ano_trim, CODUSU, NRO_HOGAR, new_role = "ID") %>%
  step_other(rama, threshold = 0.05, other = "Otro") %>%
  step_naomit(pobreza_dic) %>%
  step_rm(PONDIH) %>% 
...
test <- augment(log_reg1, new_data = test)
...
class_metrics(test, truth = pobreza_dic, estimate = .pred_class, weights = PONDIH)


# Matriz de confusión
matriz <- augment(log_reg1, new_data = train) %>% 
...
conf_mat(matriz, truth = pobreza_dic, estimate = .pred_class, case_weights = PONDIH) %>%
  ...
  labs(title = "Matriz de confusión en Train 2016 (II-III-IV) y 2017 (I)",
...
test2 <- augment(tree_rf, new_data = test)
...
class_metrics(test2, truth = pobreza_dic, estimate = .pred_class, weights = PONDIH)


# Matriz de confusión
matriz <- augment(tree_rf, new_data = train) %>% 
  select(-starts_with(".pred"), everything(), starts_with(".pred")) %>%
  mutate(.pred_class = fct_relevel(.pred_class, "Pobre", "No pobre"))
...
prevalence <- mean(matriz$pobreza_dic == "Pobre", na.rm = T)
