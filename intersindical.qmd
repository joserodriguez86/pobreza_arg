...
rm(list = ls())
pacman::p_load(tidyverse, eph, ggsci, flextable, ggrepel, GDAtools, plotly, patchwork, spatstat, lubridate, ggalluvial, jtools)
...
tabla %>% 
  ...
  geom_text(data = . %>% filter(ANO4 %in% c("2016", "2024")),
            ...
            size = 3,
            nudge_y = .02) +
  labs(title = "Estrategias económicas familiares",
...
export_summs(modelo2, exp = T, stars = c(`***` = 0.001, `**` = 0.01, `*` = 0.05, `+` = 0.1),
...
```{r Análisis de regresión multinomial salida, message=FALSE, warning=FALSE, echo=FALSE}
library(nnet)
library(huxtable)
...
```{r Análisis de regresión multinomial entrada, message=FALSE, warning=FALSE, echo=FALSE}

pool_trans <- pool %>%
  filter(Periodo == "2023 Q4", !is.na(pobreza_f_t1) & !is.na(pobreza_f), ESTADO == 1) %>%
  mutate(transicion = case_when(
    pobreza_f == "Pobre"     & pobreza_f_t1 == "Pobre"     ~ "Pobre-Pobre",
    pobreza_f == "Pobre"     & pobreza_f_t1 == "No pobre"  ~ "Pobre-NoPobre",
    pobreza_f == "No pobre"  & pobreza_f_t1 == "Pobre"     ~ "NoPobre-Pobre",
