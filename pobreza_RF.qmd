...
eph_mod <- eph_mod %>% 
  select(ano_trim, CODUSU, NRO_HOGAR, sexo_f, edad, salud_f, migrante_f, clima_educ, cant_menores, ocupados, 
         cobhe_f, pobreza_dic, empleos_cant, intensi, rama, precariedad)
...
#Preprocesamiento
recetas1 <- recipe(pobreza_dic ~ ., data = train_20171) %>%
...
recetas2 <- recipe(pobreza_dic ~ ., data = train_20171) %>%
  update_role(ano_trim, CODUSU, NRO_HOGAR, new_role = "ID") %>%
  step_other(rama, threshold = 0.02, other = "Otro") %>%
  step_relevel(pobreza_dic, ref_level = "No pobre") %>% 
...
```{r Evaluación del modelo, message=FALSE, warning=FALSE}
#Predicciones
predicciones <- augment(log_reg2, new_data = train_20171) %>% 
...
metrica_train_log <- predicciones %>%
...
conf_mat(predicciones, truth = pobreza_dic, estimate = .pred_class) %>%
