...
rm(list = ls())
pacman::p_load(tidyverse, eph, ggsci, flextable, ggrepel, GDAtools, plotly)
...
eph <- eph %>% 
  mutate(decil_ipcf = as.numeric(DECCFR),
         pobreza3 = factor(pobreza3, labels = c("Indigente", "Pobre", "No pobre")))
...
```{r Evolución de la pobreza, message=FALSE, warning=FALSE, echo=FALSE}
eph %>%
  ...
  mutate(porcentaje = case_when(situacion == "porcentaje_pobreza" & ano_sem == "2024-1" ~ 0.529,
                                situacion == "porcentaje_indigencia" & ano_sem == "2024-1" ~ 0.181,
                                TRUE ~ porcentaje)) %>%
  ggplot(mapping = aes(x = as.character(ano_sem), y = porcentaje, colour = situacion,
                       group = situacion)) +
  geom_point(size = 2) +
  geom_line(linewidth = .7) +
  geom_text(data = . %>% filter(ano_sem == "2024-1" | ano_sem == "2016-2"), 
...
ggsave("salidas/pobreza_evolucion.jpg", dpi = 300, width = 8, height = 5, create.dir = TRUE)
...
ggsave("salidas/pobreza_evolucion_trimestres.jpg", dpi = 300, width = 8, height = 5)
...
ggsave("salidas/gini_evolucion.jpg", dpi = 300, width = 8, height = 5)
```
...
ggsave("salidas/desocupacion_evolucion.jpg", dpi = 300, width = 8, height = 5)
```
...
ggsave("salidas/pobreza_intensi.jpg", dpi = 300, width = 8, height = 5)
...
ggsave("salidas/pobreza_precariedad.jpg", dpi = 300, width = 8, height = 5)
...
ggsave("salidas/pobreza_clase.jpg", dpi = 300, width = 8, height = 5)
```
...
contrib2017_1 <- tabcontrib(mca2017, dim = 1, best = T)
contrib2017_2 <- tabcontrib(mca2017, dim = 2, best = T)

ggcloud_variables(mca2017, vlab = F, shapes = T, shapesize = 1.5, force = 2,
                  textsize = 2) +
  ...
  labs(title = "Análisis de correspondencias múltiples.", 
       subtitle = "Argentina urbana 2017 (primer semestre). Población ocupada mayor a 18 años",
       caption = "Fuente: elaboración propia en base a EPH-INDEC.") +
...
ggsave(filename = "salidas/acm2017.png", dpi = 300, type = "cairo", width = 8, height = 5)


var2023 <- eph %>% 
  filter(!is.na(cobhe_f) & CH06 >= 18 & ano_sem == "2023-1" & !is.na(pobreza_dic), ESTADO == 1) %>%
  select(nivel_educativo, empleos_cant, intensi, rama, precariedad, cobhe_f, decil_ipcf, pobreza3, PONDIH)

mca2023 <- speMCA(var2023[,1:8], ncp = 2, row.w = var2023$PONDIH)

mca2023 <- flip.mca(mca2023, dim = c(1, 2))

modif.rate(mca2023)$modif 

contrib2023_1 <- tabcontrib(mca2023, dim = 1, best = T)
contrib2023_2 <- tabcontrib(mca2023, dim = 2, best = T)

ggcloud_variables(mca2023, vlab = F, shapes = T, shapesize = 1.5, force = 2,
                  textsize = 2) +
  scale_color_d3(labels= str_wrap(c("Nivel educativo",
                                    "Cantidad de empleos",
                                    "Intensidad laboral", 
                                    ...
                                    "Pobreza"))) +
  labs(title = "Análisis de correspondencias múltiples.", 
       subtitle = "Argentina urbana 2023 (primer semestre). Población ocupada mayor a 18 años",
       caption = "Fuente: elaboración propia en base a EPH-INDEC.") +
  theme(legend.position = "right",
        legend.title = element_blank(),
        legend.text = element_text(size = 8),
...
ggsave(filename = "salidas/acm2023.png", dpi = 300, type = "cairo", width = 8, height = 5)
...
cloud_ind_2023 <- ggcloud_indiv(mca2023, col = "lightgrey")
cloud_ind_2023 <- ggadd_kellipses(cloud_ind_2023, mca2023, var2023$pobreza3, label = FALSE) +
  scale_color_d3() +
  labs(title = "2023") +
  theme(legend.position = "right",
        legend.title = element_blank(),
...
ggsave("salidas/distancias_pobreza_densidad.jpg", dpi = 300, width = 8, height = 5)
...
ggsave("salidas/distancias_pobreza.jpg", dpi = 300, width = 8, height = 5)
...
ggsave("salidas/distancias_pobreza2.jpg", dpi = 300, width = 8, height = 5)
```
