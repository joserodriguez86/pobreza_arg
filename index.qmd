...
rm(list = ls())
pacman::p_load(tidyverse, eph, ggsci, flextable, ggrepel, GDAtools)
...
eph %>%
  ...
  geom_text(data = . %>% filter(ano_sem == "2024-1"), 
...
eph <- eph %>% 
  group_by(ano_sem, CODUSU, NRO_HOGAR) %>%
  mutate(miembros = n()) %>% 
  ungroup() %>% 
...
eph %>%
  ...
  ggplot(aes(x = as.character(ano_sem), y = gini, group = 1)) +
  geom_point(size = 2) +
  geom_line(linewidth = .7) +
